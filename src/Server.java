...
public class Server implements MessageListener {
	...
	/**
	 * Opens the TCP port on the current machine for listening. initializes the
	 * linked lists.
	 * 
	 * @param port
	 */
	...
	public void messageReceived(String message, int clientID) {
		System.out.println("Message receieved= " + message + " clientID= "
				+ clientID);
	...
	public void FCFS(String message, int clientID) {
		...
		if (message.substring(0, 7).equals("REQUEST")) {
			...
			if (volunteer.size() == 0) { // if there is no volunteers, add to
											// queue
				...
				try {
					channel.sendMessage(
							...
							volID.removeFirst());
					channel.sendMessage("VOLUNTEER " + volunteer.removeFirst(),
							clientID);
			...
			if (requests.size() == 0) {
				volID.add(clientID); // adds the client id of the volunteer to a
										// list
				volunteer.add(message.substring(10, message.length()));
			} else if (requests.size() > 0) {
				try {
					channel.sendMessage("LOCATION " + requests.removeFirst(),
							clientID);
					channel.sendMessage(
							"VOLUNTEER "
	...
	/**
	 * constructs a new Server with the port at position args[0] ex. java Server
	 * 8888 runs with the port 8888
	 */
